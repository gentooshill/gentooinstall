...
def chroot_env():
    ...
    # Write the ensure_build_dirs.py script to /mnt/gentoo/root, line by line for robustness
    ensure_script_lines = [
        "#!/usr/bin/env python3\n",
        "import os\n",
        "import subprocess\n\n",
        "def is_disk_mount(path):\n",
        "    try:\n",
        "        out = subprocess.check_output(['findmnt', '-n', '-o', 'FSTYPE', path], text=True).strip()\n",
        "        return out not in ('tmpfs', 'ramfs', 'overlay')\n",
        "    except Exception:\n",
        "        return False\n\n",
        "def check_and_fix(path):\n",
        "    if not is_disk_mount(path):\n",
        "        print(f'[WARN] {path} is not on a real disk. Forcibly unmounting and recreating.')\n",
        "        subprocess.run(['umount', '-lf', path], check=False)\n",
        "        if not os.path.exists(path):\n",
        "            os.makedirs(path, exist_ok=True)\n",
        "        else:\n",
        ...
        "        os.chmod(path, 0o1777)\n",
        "    else:\n",
        "        print(f'[OK] {path} is on a real disk.')\n\n",
        "def main():\n",
        "    for path in ['/var/tmp', '/tmp', '/var/tmp/portage']:\n",
        "        check_and_fix(path)\n",
        "    make_conf = '/etc/portage/make.conf'\n",
        "    if os.path.exists(make_conf):\n",
        "        with open(make_conf, 'r') as f:\n",
        ...
        "        with open(make_conf, 'w') as f:\n",
        "            f.writelines(lines)\n",
        "        print('[OK] Ensured PORTAGE_TMPDIR is set to /var/tmp in make.conf')\n",
        "    statvfs = os.statvfs('/')\n",
        "    free_gb = statvfs.f_frsize * statvfs.f_bavail / (1024**3)\n",
        "    if free_gb < 10:\n",
        "        print(f'[FATAL] Root filesystem has only {free_gb:.1f}GB free. At least 10GB is required.')\n",
        "        exit(1)\n",
        "    print(f'[OK] Root filesystem has {free_gb:.1f}GB free.')\n",
        "    try:\n",
        "        df_out = subprocess.check_output(['df', '-i', '/'], text=True).splitlines()\n",
        "        if len(df_out) >= 2:\n",
        "            avail_inodes = int(df_out[1].split()[3])\n",
        "            if avail_inodes < 10000:\n",
        "                print(f'[FATAL] Only {avail_inodes} inodes free on /. At least 10000 required.')\n",
        "                exit(1)\n",
        "            print(f'[OK] {avail_inodes} inodes free on /.')\n",
        "    except Exception as e:\n",
        "        print(f'[WARN] Could not check inodes: {e}')\n",
        "    try:\n",
        "        with open('/proc/mounts') as f:\n",
        "            for line in f:\n",
        "                parts = line.split()\n",
        "                if len(parts) >= 3 and parts[1].startswith(('/var/tmp', '/tmp')):\n",
        "                    if parts[2] in ('tmpfs', 'ramfs', 'overlay'):\n",
        "                        print(f'[FATAL] {parts[1]} is mounted as {parts[2]}. Aborting.')\n",
        "                        exit(1)\n",
        "    except Exception as e:\n",
        "        print(f'[WARN] Could not check /proc/mounts: {e}')\n",
        "    print('[ALL CHECKS PASSED] All build/temp dirs are on disk, with sufficient space and inodes.')\n\n",
        "if __name__ == '__main__':\n",
        "    main()\n"
    ]
    with open("/mnt/gentoo/root/ensure_build_dirs.py", "w") as f:
        f.writelines(ensure_script_lines)
    os.chmod("/mnt/gentoo/root/ensure_build_dirs.py", 0o755)
    # After chroot, run the ensure script
    run_cmd("chroot /mnt/gentoo /bin/bash -c 'python3 /root/ensure_build_dirs.py'")
    pause()
